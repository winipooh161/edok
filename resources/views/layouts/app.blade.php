...
<head>
    ...
    @hasSection('meta_tags')
        @yield('meta_tags')
    @else
        <title>{{ config('app.name', 'Laravel') }} - Кулинарная книга</title>
  ...
  />
    ...
    @hasSection('meta_tags')
    @else
        <meta property="og:title" content="{{ config('app.name') }} - Кулинарная книга">
...
">+7 904 448-22-83
                ...
                <div class="row mt-3">
                    <div class="col-12 text-center">
                        <p class="mb-0 text-muted">&copy; {{ date('Y') }} Я едок. Все права защищены.</p>
                    </div>
                </div>
            </div>
        </footer>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (searchInput) {
                    initAutocomplete(searchInput, '{{ route('search.autocomplete') }}');
                }
                    ...
                    searchResultLinks.forEach(link => {
                        link.addEventListener('click', function(e) {
                            ...
                            if (recipeId && searchQuery) {
                                ...
                                fetch('{{ route('search.record-click') }}', {
                                    method: 'POST',
                                    headers: {
                                ...
                                .then(data => {
                                    ...
                                    if (data.length > 0) {
                                        ...
                                        showAllElement.innerHTML = `
                                            <a href="{{ route('search') }}?query=${encodeURIComponent(query)}" class="d-flex align-items-center p-2 text-decoration-none text-primary">
