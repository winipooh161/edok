            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            ...
                            <a href="{{ route('admin.parser.collectLinksForm') }}" class="btn btn-outline-secondary">
                ...
                <div class="card-body">
                    <div class="alert alert-info mb-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p class="mb-0">Найдено ссылок на рецепты: <strong>{{ $totalLinks }}</strong></p>
                                @if(session()->has('unique_links'))
                                    <p class="mb-0">Из них уникальных (отсутствуют в базе): <strong>{{ count(session('unique_links', [])) }}</strong></p>
                                @endif
                                @if(isset($totalImages))
                                <p class="mb-0">Найдено изображений: <strong>{{ $totalImages }}</strong></p>
                                @endif
                            </div>
                            <div class="text-end">
                                ...
                                @if(isset($collectedImages) && count($collectedImages) > 0)
                    ...
                    <div class="mb-4">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                <tr>
                                    <th>URL категории:</th>
                                    <td><a href="{{ $categoryUrl }}" target="_blank">{{ $categoryUrl }}</a></td>
                                </tr>
                                <tr>
                                    <th>Обработано страниц:</th>
                                    <td>{{ count($processedUrls) }}</td>
                                </tr>
                                <tr>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                <h6>Статистика:</h6>
                                <ul>
                                    <li>Всего найдено ссылок: <strong>{{ count($collectedLinks) + count(session('duplicate_links', [])) }}</strong></li>
                                    <li>Новых ссылок для обработки: <strong>{{ count($collectedLinks) - count(array_intersect($collectedLinks, session('duplicate_links', []))) }}</strong></li>
                                    @if(count(session('duplicate_links', [])) > 0)
                                        <li>Пропущено ссылок (уже есть в базе): <strong>{{ count(session('duplicate_links', [])) }}</strong></li>
                                    @endif
                                    @if(session()->has('unique_links'))
                                        <li>Из них гарантированно уникальных (отсутствуют в базе): <strong>{{ count(session('unique_links', [])) }}</strong></li>
                                    @endif
                                </ul>
                            </div>
                                    ...
                                    <div class="collapse" id="duplicateLinksCollapse">
                                        <div class="card card-body bg-light">
                                            ...
                                            <div style="max-height: 200px; overflow-y: auto;">
                                                <ol class="mb-0">
                                                    @foreach(session('duplicate_links', []) as $url)
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-hover" id="linksTable">
                                    ...
                                    <tbody>
                                        @foreach($collectedLinks as $index => $link)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <button class="btn btn-sm btn-outline-secondary btn-copy-link" data-url="{{ $link }}">
                                                    <i class="fas fa-copy"></i>
                                                </button>
                                                <a href="{{ route('admin.parser.index', ['url' => $link]) }}" class="btn btn-sm btn-outline-primary">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped" id="imagesTable">
                            ...
                            <tbody>
                                @foreach($collectedImages as $index => $image)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td class="image-preview-cell">
                                        <img src="{{ $image }}" alt="Превью" class="img-thumbnail image-preview" style="max-height: 60px; display: none;">
                                        <span class="badge bg-secondary image-placeholder">Превью</span>
                                    </td>
                                    <td class="image-url">{{ $image }}</td>
                                    <td>
                                        <button class="btn btn-sm btn-outline-secondary btn-copy-image" data-url="{{ $image }}">
                                            <i class="fas fa-copy"></i>
                                        </button>
                                        <a href="{{ $image }}" target="_blank" class="btn btn-sm btn-outline-primary">
                    ...
                    <div class="table-responsive">
                        <table class="table table-danger">
                            ...
                            <tbody>
                                @foreach($failedUrls as $index => $failedUrl)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $failedUrl['url'] }}</td>
                                    <td>{{ $failedUrl['error'] }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="card-footer">
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('admin.parser.collectLinksForm') }}" class="btn btn-secondary">
                            <i class="fas fa-arrow-left"></i> Вернуться к форме сбора
                        </a>
                        <a href="{{ route('admin.parser.batch') }}" class="btn btn-primary">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (btnSendToBatch) {
            btnSendToBatch.addEventListener('click', function() {
                ...
                @if(session()->has('unique_links'))
                    const uniqueLinks = {!! json_encode(session('unique_links')) !!};
                ...
                window.location.href = '{{ route("admin.parser.batch") }}';
            });
        }
